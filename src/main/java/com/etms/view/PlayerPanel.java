package com.etms.view;

import com.etms.controller.DashboardController;
import com.etms.model.Player;
import com.etms.theme.ThemeManager;
import com.etms.theme.Typography;
import com.etms.theme.Spacing;
import com.etms.ui.components.ETMSButton;
import com.etms.ui.components.ETMSPageHeader;
import com.etms.ui.components.ETMSTable;
import net.miginfocom.swing.MigLayout;

import javax.swing.*;
import javax.swing.table.DefaultTableModel;
import java.awt.*;
import java.util.List;

public class PlayerPanel extends JPanel {

    private final DashboardController controller;
    private ETMSTable playerTable;
    private DefaultTableModel tableModel;
    private JTextField searchField;
    private ETMSButton addBtn, refreshBtn, deleteBtn, editBtn;

    public PlayerPanel(DashboardController controller) {
        this.controller = controller;
        setBackground(ThemeManager.getBackground());
        setLayout(new MigLayout("fill, insets 20", "[grow]", "[][][grow][]"));
        initComponents();
        loadPlayers();
    }

    private void initComponents() {
        ETMSPageHeader header = new ETMSPageHeader("Players", "Manage player profiles.");
        add(header, "growx, wrap, gapbottom " + Spacing.LG);

        JPanel toolbar = new JPanel(new MigLayout("fillx, insets 0", "[grow][]", "[]"));
        toolbar.setOpaque(false);
        searchField = new JTextField();
        searchField.putClientProperty("JTextField.placeholderText", "Search players...");
        searchField.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent e) { filterTable(); }
        });
        toolbar.add(searchField, "width 200!");

        refreshBtn = new ETMSButton("Refresh", ETMSButton.Variant.SECONDARY);
        refreshBtn.addActionListener(e -> loadPlayers());
        toolbar.add(refreshBtn, "gapleft " + Spacing.SM);

        addBtn = new ETMSButton("+ Add Player", ETMSButton.Variant.PRIMARY);
        addBtn.addActionListener(e -> openAddDialog());
        toolbar.add(addBtn, "gapleft " + Spacing.SM + ", align right");

        add(toolbar, "growx, wrap, gapbottom " + Spacing.LG);

        String[] columns = {"ID", "Name", "In-Game Name", "Rank", "Role", "Team"};
        tableModel = new DefaultTableModel(columns, 0) {
            @Override public boolean isCellEditable(int row, int col) { return false; }
        };
        playerTable = new ETMSTable(tableModel);
        JScrollPane scroll = new JScrollPane(playerTable);
        add(scroll, "grow, wrap, gapbottom " + Spacing.LG);

        JPanel actionPanel = new JPanel(new MigLayout("fillx, insets 0", "[][][]", "[]"));
        actionPanel.setOpaque(false);
        editBtn = new ETMSButton("Edit", ETMSButton.Variant.SECONDARY);
        editBtn.addActionListener(e -> editSelected());
        actionPanel.add(editBtn);

        deleteBtn = new ETMSButton("Delete", ETMSButton.Variant.DANGER);
        deleteBtn.addActionListener(e -> deleteSelected());
        actionPanel.add(deleteBtn);

        add(actionPanel, "growx");
    }

    private void loadPlayers() {
        tableModel.setRowCount(0);
        List<Player> players = controller.getAllPlayers();
        for (Player p : players) {
            String fullName = p.getFirstName() + " " + p.getLastName();
            String teamName = p.getTeamName() != null ? p.getTeamName() : "None";
            tableModel.addRow(new Object[]{
                p.getPlayerId(),
                fullName,
                p.getInGameName(),
                p.getGameRank() != null ? p.getGameRank() : "Unranked",
                p.getGameRole() != null ? p.getGameRole() : "Flex",
                teamName
            });
        }
    }

    private void filterTable() {
        String search = searchField.getText().trim().toLowerCase();
        tableModel.setRowCount(0);
        for (Player p : controller.getAllPlayers()) {
            String fullName = (p.getFirstName() + " " + p.getLastName()).toLowerCase();
            String ign = p.getInGameName() != null ? p.getInGameName().toLowerCase() : "";
            if (!search.isEmpty() && !fullName.contains(search) && !ign.contains(search)) continue;
            String teamName = p.getTeamName() != null ? p.getTeamName() : "None";
            tableModel.addRow(new Object[]{
                p.getPlayerId(),
                p.getFirstName() + " " + p.getLastName(),
                p.getInGameName(),
                p.getGameRank() != null ? p.getGameRank() : "Unranked",
                p.getGameRole() != null ? p.getGameRole() : "Flex",
                teamName
            });
        }
    }

    private void openAddDialog() {
        PlayerEditDialog dialog = new PlayerEditDialog(
            (JFrame) SwingUtilities.getWindowAncestor(this), null);
        dialog.setVisible(true);
        if (dialog.isConfirmed()) {
            Player player = dialog.getUpdatedPlayer();
            boolean success = controller.addPlayer(player);
            if (success) {
                JOptionPane.showMessageDialog(this, "Player added successfully.", "Success", JOptionPane.INFORMATION_MESSAGE);
                loadPlayers();
            } else {
                loadPlayers();
            }
        }
    }

    private void editSelected() {
        int row = playerTable.getSelectedRow();
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Select a player to edit.");
            return;
        }
        int id = (Integer) tableModel.getValueAt(row, 0);
        Player player = controller.getAllPlayers().stream()
            .filter(p -> p.getPlayerId() == id)
            .findFirst()
            .orElse(null);
        if (player != null) {
            PlayerEditDialog dialog = new PlayerEditDialog(
                (JFrame) SwingUtilities.getWindowAncestor(this), player);
            dialog.setVisible(true);
            if (dialog.isConfirmed()) {
                Player updated = dialog.getUpdatedPlayer();
                boolean success = controller.updatePlayer(updated);
                if (success) {
                    JOptionPane.showMessageDialog(this, "Player updated successfully.", "Success", JOptionPane.INFORMATION_MESSAGE);
                    loadPlayers();
                } else {
                    loadPlayers();
                }
            }
        }
    }

    private void deleteSelected() {
        int row = playerTable.getSelectedRow();
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Select a player to delete.");
            return;
        }
        int id = (Integer) tableModel.getValueAt(row, 0);
        if (JOptionPane.showConfirmDialog(this, "Delete this player?") == JOptionPane.YES_OPTION) {
            if (controller.deletePlayer(id)) {
                loadPlayers();
                JOptionPane.showMessageDialog(this, "Player deleted.");
            } else {
                JOptionPane.showMessageDialog(this, "Deletion failed.");
            }
        }
    }

    public void refreshTheme() {
        setBackground(ThemeManager.getBackground());

        for (Component comp : getComponents()) {
            if (comp instanceof JPanel) {
                comp.setBackground(ThemeManager.getBackground());
            }
            if (comp instanceof JScrollPane) {
                comp.setBackground(ThemeManager.getBackground());
                ((JScrollPane) comp).getViewport().setBackground(ThemeManager.getSurface());
            }
        }

        if (playerTable != null) playerTable.refreshTheme();
        if (refreshBtn != null) refreshBtn.refreshTheme();
        if (addBtn != null) addBtn.refreshTheme();
        if (editBtn != null) editBtn.refreshTheme();
        if (deleteBtn != null) deleteBtn.refreshTheme();

        revalidate();
        repaint();
    }
}